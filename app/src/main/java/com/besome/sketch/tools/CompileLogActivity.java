...
public class CompileLogActivity extends BaseAppCompatActivity {
    ...
    private void openErrorLogAi(String scId) {
        ...
        final String deduplicated = deduplicateErrors(rawLog);
        // Only ERROR-severity lines go to the AI payload below (systemPrompt and
        // Tool.ai bodies); WARNING/NOTE lines are still visible to the user in the
        // regular on-screen log viewer, they're just excluded here to keep the
        // agent focused and reduce token usage (see filterErrorsOnly()).
        final String errors = filterErrorsOnly(deduplicated);
    ...
    /**
     * Filters a deduplicated log down to ERROR-severity lines only, using the
     * existing pro.sketchware.utility.diagnostics.CompileDiagnostic.Severity
     * infrastructure (previously parsed but never applied to the AI payload).
     * WARNING/NOTE lines are dropped from what's sent to the AI to cut noise and
     * token usage, while the raw/deduplicated log keeps showing everything in the
     * on-screen log viewer (this method is only used for the AI payload).
     *
     * Lines the parser doesn't recognize (e.g. stack trace continuation lines,
     * "Caused by:", blank separators) are kept as-is so we never silently drop
     * context the AI needs — only lines that were positively identified as
     * WARNING or NOTE are removed.
     */
    private String filterErrorsOnly(String deduplicatedErrors) {
        if (deduplicatedErrors == null || deduplicatedErrors.isEmpty()) return "";

        java.util.List<pro.sketchware.utility.diagnostics.CompileDiagnostic> diagnostics =
                pro.sketchware.utility.diagnostics.CompileErrorCapture.parse(deduplicatedErrors);

        if (diagnostics.isEmpty()) {
            // Parser didn't recognize any javac/kotlinc-style diagnostic lines
            // (e.g. a raw stack trace or a build-tool message in a different format).
            // Nothing to safely filter, so keep the log as-is rather than risk
            // dropping the only information available.
            return deduplicatedErrors;
        }

        java.util.Set<String> nonErrorRawLines = new java.util.HashSet<>();
        for (pro.sketchware.utility.diagnostics.CompileDiagnostic diagnostic : diagnostics) {
            if (diagnostic.severity != pro.sketchware.utility.diagnostics.CompileDiagnostic.Severity.ERROR) {
                nonErrorRawLines.add(diagnostic.raw.trim());
            }
        }

        if (nonErrorRawLines.isEmpty()) {
            return deduplicatedErrors;
        }

        StringBuilder filtered = new StringBuilder();
        for (String line : deduplicatedErrors.split("\\n")) {
            if (nonErrorRawLines.contains(line.trim())) {
                continue;
            }
        ...
        // Never send an empty payload just because everything happened to be
        // warnings/notes (e.g. a "successful but noisy" build) — fall back to the
        // full deduplicated log so the AI still has something to work with.
        return result.trim().isEmpty() ? deduplicatedErrors : result;
    }
