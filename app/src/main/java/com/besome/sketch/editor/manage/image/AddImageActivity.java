...
public class AddImageActivity extends BaseDialogActivity implements View.OnClickListener {
    ...
    private void setImageFromUri(Uri uri) {
        if (uri == null) return;
        String filePath = HB.a(this, uri);
        if (filePath != null) {
            setImageFromFile(filePath);
            return;
        }
        // ContentResolver fallback for SAF/MediaStore URIs where HB.a() returns null
        try {
            String ext = ".png";
            ...
            File tmp = File.createTempFile("picked_img_", ext, getCacheDir());
            ...
            setImageFromFile(tmp.getAbsolutePath());
        } catch (Exception e) {
            e.printStackTrace();
            bB.b(this, "Failed to load image: " + e.getMessage(), bB.TOAST_WARNING).show();
        }
    }
                ...
                if (!activity.multipleImagesPicked) {
                    ...
                    while (i < activity.pickedImageUris.size()) {
                        ...
                        if (imageFilePath == null) {
                            // ContentResolver fallback for SAF URIs
                            try {
                                String ext = ".png";
                                ...
                                java.io.File tmp = java.io.File.createTempFile(
                                        "multi_img_" + i + "_", ext, activity.getCacheDir());
                                try (java.io.InputStream in2 = activity.getContentResolver().openInputStream(uri);
                                     java.io.FileOutputStream out2 = new java.io.FileOutputStream(tmp)) {
                                    if (in2 == null) continue;
                                    byte[] buf2 = new byte[8192]; int n2;
                                    while ((n2 = in2.read(buf2)) != -1) out2.write(buf2, 0, n2);
                                }
                                imageFilePath = tmp.getAbsolutePath();
                            } catch (Exception ex) {
                                ex.printStackTrace();
                                continue;
                            }
