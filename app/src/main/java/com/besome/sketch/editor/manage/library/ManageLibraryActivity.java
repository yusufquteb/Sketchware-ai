...
import pro.sketchware.activities.library.extras.DaydreamToolsActivity;
import pro.sketchware.activities.library.extras.DaydreamToolsNavItemView;
...
public class ManageLibraryActivity extends BaseAppCompatActivity implements View.OnClickListener {
    ...
    private final int REQUEST_CODE_DAYDREAM_TOOLS_ACTIVITY = 244;

    // Nav-item tag constant (must not collide with PROJECT_LIB_TYPE_* values 0–8)
    private static final int NAV_DAYDREAM_TOOLS = 100;
    ...
    private void toDaydreamToolsActivity() {
        boolean firebaseEnabledNow = firebaseLibraryBean != null
                && ProjectLibraryBean.LIB_USE_Y.equals(firebaseLibraryBean.useYn);
        boolean appCompatEnabledNow = compatLibraryBean != null
                && ProjectLibraryBean.LIB_USE_Y.equals(compatLibraryBean.useYn);

        Intent intent = new Intent(getApplicationContext(), DaydreamToolsActivity.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        intent.putExtra(DaydreamToolsActivity.EXTRA_SC_ID, sc_id);
        intent.putExtra(DaydreamToolsActivity.EXTRA_FIREBASE_ENABLED, firebaseEnabledNow);
        intent.putExtra(DaydreamToolsActivity.EXTRA_APP_COMPAT_ENABLED, appCompatEnabledNow);
        startActivityForResult(intent, REQUEST_CODE_DAYDREAM_TOOLS_ACTIVITY);
    }
    ...
    /**
     * Core save + exit logic. Called by both the toolbar ← button and the
     * OnBackPressedCallback registered in onCreate().
     */
    private void handleBackPress() {
        android.util.Pair<Boolean, java.util.List<BuiltInLibraries.BuiltInLibrary>> excludedLibrariesConfig =
                ExcludeBuiltInLibrariesActivity.readConfigCompat(sc_id);
        BuiltInLibraryCompatibilityMatrix.ValidationResult validationResult =
                BuiltInLibraryCompatibilityMatrix.validate(sc_id, compatLibraryBean,
                        firebaseLibraryBean, admobLibraryBean, googleMapLibraryBean,
                        excludedLibrariesConfig.first, excludedLibrariesConfig.second);
        ...
        if (!mB.a()) {
            ...
            if (tag != null) {
                ...
                switch (vTag) {
                    ...
                    case NAV_DAYDREAM_TOOLS:
                        toDaydreamToolsActivity();
                        break;
                }
            }
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // On targetSdk >= 33 the system routes back-press through OnBackPressedDispatcher,
        // bypassing @Override onBackPressed(). This callback ensures handleBackPress()
        // is always called regardless of Android version.
    ...
    public void onPostCreate(Bundle savedInstanceState) {
        ...
        addLibraryItem(new ProjectLibraryBean(ProjectLibraryBean.PROJECT_LIB_TYPE_NATIVE_LIB), externalCategory);

        // ── daydream tools ────────────────────────────────────────────────
        // Opens a dedicated page (DaydreamToolsActivity) with three titled
        // cards ported from daydream's LibrarySettings / GoogleSettings /
        // UISettings screens: Firebase & Google, User Interface, Extra
        // Libraries.
        DaydreamToolsNavItemView daydreamToolsNavItem = new DaydreamToolsNavItemView(this);
        daydreamToolsNavItem.setTag(NAV_DAYDREAM_TOOLS);
        daydreamToolsNavItem.setOnClickListener(this);
        externalCategory.addLibraryItem(daydreamToolsNavItem, false);
        libraryItems.add(daydreamToolsNavItem);
