...
public final class AiCapabilityManifest {
    ...
    static {
        ...
        activities.add(new ToolEntry("seed_blank_activity_content", "Fill a newly created activity with starter view/logic/layout content", CAT_ACTIVITIES, "ic_mtrl_add_circle"));
        ...
        ui.add(new ToolEntry("rtl_audit_project",       "Deterministic project-wide RTL audit (no LLM)", CAT_UI, "ic_mtrl_format_textdirection_r_to_l"));
        ui.add(new ToolEntry("material_audit_project",  "Deterministic project-wide Material color audit (no LLM)", CAT_UI, "ic_mtrl_palette"));
        ...
        build.add(new ToolEntry("run_and_verify_on_device", "Install, launch, and check for crash-on-launch (requires root)", CAT_BUILD, "ic_mtrl_play_circle"));
        ...
        search.add(new ToolEntry("web_search",                "Search the internet for current info, docs, or examples", CAT_SEARCH, "ic_mtrl_search"));
        ...
        List<ToolEntry> git = new ArrayList<>();
        git.add(new ToolEntry("git_init",   "Initialize a local git repo for the project", CAT_PROJECT, "ic_mtrl_account_tree"));
        git.add(new ToolEntry("git_status", "Show working tree status",                    CAT_PROJECT, "ic_mtrl_info"));
        git.add(new ToolEntry("git_add",    "Stage files for commit",                      CAT_PROJECT, "ic_mtrl_add_box"));
        git.add(new ToolEntry("git_commit", "Commit staged changes",                       CAT_PROJECT, "ic_mtrl_deployed_code"));
        REGISTRY.get(CAT_PROJECT).addAll(git);

        // ── Snapshots (project-level undo/checkpoints) ─────────────────────────
        List<ToolEntry> snapshots = new ArrayList<>();
        snapshots.add(new ToolEntry("list_snapshots",   "List saved project snapshots",      CAT_PROJECT, "ic_mtrl_history"));
        snapshots.add(new ToolEntry("create_snapshot",  "Save a restorable project snapshot", CAT_PROJECT, "ic_mtrl_save"));
        snapshots.add(new ToolEntry("restore_snapshot", "Roll the project back to a snapshot", CAT_PROJECT, "ic_mtrl_undo"));
        REGISTRY.get(CAT_PROJECT).addAll(snapshots);
