...
public class AnthropicApiClient extends AiApiClient {
    ...
    public void sendChatRequest(List<ChatMessage> messages, String modelId,
        ...
        try {
            ...
            body.addProperty("max_tokens", userMaxTokens > 0 ? userMaxTokens : 8192);
            // Anthropic requires temperature in [0,1]; only send it when the user explicitly
            // configured one (0 = "not configured" sentinel, see AiApiClient.userTemperature).
            if (userTemperature > 0f) body.addProperty("temperature", Math.min(1f, userTemperature));
