...
public class GeminiApiClient extends AiApiClient {
    ...
    private JsonObject buildRequestBody(List<ChatMessage> messages, String systemPrompt,
        ...
        // ✅ FIX: forward user-configured temperature/max_tokens (AiPreferences, "AI Performance
        // Profiles" screen) — previously never sent, so the setting had no runtime effect at all.
        // 0 = not configured by the user; Gemini keeps its own provider default in that case.
        if (userTemperature > 0f || userMaxTokens > 0) {
            JsonObject generationConfig = new JsonObject();
            if (userTemperature > 0f) generationConfig.addProperty("temperature", userTemperature);
            if (userMaxTokens > 0) generationConfig.addProperty("maxOutputTokens", userMaxTokens);
