...
public final class AiProjectIntegrationHelper {
    ...
    @NonNull
    public static Conversation createConversation(@NonNull Context context, @NonNull Workspace workspace,
                                                    @Nullable String title) {
        return createConversation(context, workspace, title, null);
    }

    /**
     * @param scId if non-null, persisted on the Conversation so that reopening it later
     *             (e.g. from WorkspaceConversationsFragment) still knows which single
     *             project it's scoped to, instead of silently falling back to
     *             SCOPE_GLOBAL across the whole workspace. Previously scId was only
     *             ever passed transiently via the Intent extra on first open and never
     *             saved, so a project-scoped chat would describe a different/random
     *             project's data after being reopened from the conversation list.
     */
    @NonNull
    public static Conversation createConversation(@NonNull Context context, @NonNull Workspace workspace,
                                                    @Nullable String title, @Nullable String scId) {
        ConversationManager conversationManager = new ConversationManager(context);
        Conversation conversation = new Conversation(workspace.getId(),
                title == null || title.trim().isEmpty() ? "New Chat" : title.trim(), null, null, scId);
    ...
    public static void openProjectChat(@NonNull Activity activity, @NonNull String scId, @Nullable String projectName,
        ...
        Conversation conversation = createConversation(activity, workspace, title, scId);
