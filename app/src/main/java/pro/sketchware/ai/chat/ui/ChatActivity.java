...
import pro.sketchware.ai.voice.VoiceInputManager;
...
public class ChatActivity extends AppCompatActivity
    ...
    @Nullable private VoiceInputManager voiceInputManager;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        voiceInputManager = new VoiceInputManager(this, VoiceInputManager.Language.AUTO);
        voiceInputManager.setCallback(new VoiceInputManager.Callback() {
            @Override public void onListening() {
                if (btnMic != null) btnMic.setSelected(true);
            }
            @Override public void onPartialResult(String partialText) {
                if (inputEditText != null) inputEditText.setText(partialText);
            }
            @Override public void onFinalResult(String finalText) {
        ...
        // Fix: EXTRA_PROJECT_ID is only ever set by the launch site that FIRST
        // created this conversation (see AiProjectIntegrationHelper). Every other
        // path that reopens an existing conversation (WorkspaceConversationsFragment,
        // AiBackgroundService's notification tap, ...) never passed it, silently
        // downgrading a project-scoped chat to SCOPE_GLOBAL across the whole
        // workspace — which is what was causing the AI to describe the wrong
        // project. conversation.getProjectId() is now the source of truth (persisted
        // at creation time), so any launch site that forgets the extra still works.
    ...
    private void showModelSelector() {
        ...
        for (AiProvider p : AiProvider.values()) {
            if (p == AiProvider.LLM7 || p == AiProvider.CHUTES || p == AiProvider.DEEPSEEK) continue; // legacy, hidden everywhere
            ...
            if (p == AiProvider.LOCAL_LLM) {
                if (hasAnyReadyLocalModel()) available.add(p);
                continue;
            }
        ...
        ModelProviderPagerAdapter pager = new ModelProviderPagerAdapter(
                available, preferences, currentModelId, this, model -> {
    ...
    private boolean hasAnyReadyLocalModel() {
        pro.sketchware.ai.offline.LocalModelManager localModelManager =
                new pro.sketchware.ai.offline.LocalModelManager(this);
    ...
    private void startSpeechInput() {
        ...
        String availability = voiceInputManager.checkAvailability();
        if (availability != null) {
            android.widget.Toast.makeText(this, availability, android.widget.Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private void showPulseConfirmation(String plan, Runnable onContinue, Runnable onCancel) {
        ...
        int pulseSecs = pro.sketchware.ai.engine.AgentExecutor.PULSE_AUTO_SECS;
        bar.setMax(pulseSecs);
        bar.setProgress(pulseSecs);
        ...
        timerTv.setText("Continuing in " + pulseSecs + "s…");
