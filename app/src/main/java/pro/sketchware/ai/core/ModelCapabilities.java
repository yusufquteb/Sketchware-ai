...
public final class ModelCapabilities {
     ...
     */
    @NonNull
    @SuppressWarnings("deprecation") // intentionally still handles the legacy LLM7 case
    public static ProviderCapabilities resolve(
        ...
        switch (provider) {
            ...
            // ── DeepSeek: deepseek-reasoner (R1) does not reliably support tool
            //    calling the way deepseek-chat (V3) does — sending "tools"/"tool_choice"
            //    to the reasoner endpoint gets silently ignored, and the model falls back
            //    to plain chain-of-thought text instead of calling the requested tool.
            case DEEPSEEK:
                return resolveDeepSeek(m, providerDefault);
    ...
    private static ProviderCapabilities resolveDeepSeek(
            String m, ProviderCapabilities base) {
        // deepseek-reasoner (R1): no reliable tool-calling support — force plain-chat mode
        // so AgentExecutor sends an empty tool list instead of a "tools" param the endpoint
        // ignores. deepseek-chat (V3, default/fallback) keeps the provider default (tools=true).
        boolean hasTools = !m.contains("reasoner");
        if (hasTools == base.supportsTools) return base;
        return new ProviderCapabilities.Builder().from(base).tools(hasTools).build();
    }
