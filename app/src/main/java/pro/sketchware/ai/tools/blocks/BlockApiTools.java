...
public final class BlockApiTools {
    ...
    public static class AddBlockTool implements AgentTool {
        ...
        // Added Phase 3: was defaulting to LOW (no override existed before), which meant
        // ToolValidator never set requiresApproval/requiresSnapshot for this tool even
        // though it mutates a project's logic file. MEDIUM matches how other file-mutating
        // tools (e.g. patch_file) are classified.
        ...
        public ToolResult execute(JsonObject args, ToolContext ctx) {
            ...
            // Phase 3: capture pre-mutation text for diff display (item 4 — connects
            // ai.engine.diff to the Blocks path). See BlockDiffSupport javadoc for why
            // this is an after-the-fact diff, not a pre-execution approval gate.
            String beforeText = pro.sketchware.ai.engine.diff.BlockDiffSupport.captureBeforeText(ctx, scId);
            BlockLogicReader.LogicFile lf = BlockLogicReader.read(logicFile(ctx, scId), scId);
            if (lf == null) return error("Could not parse logic file");
            ...
            String diffSummary = pro.sketchware.ai.engine.diff.BlockDiffSupport.buildAfterDiff(
                    ctx, scId, "add_block", beforeText);
            String msg = "Block added successfully to event '" + fullName + "' in project " + scId;
            return success(diffSummary != null ? msg + "\n" + diffSummary : msg);
        }
    }
    ...
    public static class ModifyBlockTool implements AgentTool {
        ...
        // Added Phase 3 — see AddBlockTool's identical comment for reasoning.
        @Override public RiskLevel getRiskLevel() { return RiskLevel.MEDIUM; }
        ...
        public ToolResult execute(JsonObject args, ToolContext ctx) {
            ...
            if (err != null) return error(err);
            String diffSummary = pro.sketchware.ai.engine.diff.BlockDiffSupport.buildAfterDiff(
                    ctx, scId, "modify_block", beforeText);
            String msg = "Block " + blockId + " modified in event '" + fullName + "'";
            return success(diffSummary != null ? msg + "\n" + diffSummary : msg);
        }
    }
    ...
    public static class DeleteBlockTool implements AgentTool {
        ...
        // Added Phase 3 — see AddBlockTool's identical comment for reasoning.
        // Arguably CRITICAL (destructive), but kept at MEDIUM to match the other
        // two block tools' classification rather than invent a new distinction
        // not requested by this phase's scope; flagged here for a future review.
        @Override public RiskLevel getRiskLevel() { return RiskLevel.MEDIUM; }
        ...
        public ToolResult execute(JsonObject args, ToolContext ctx) {
            ...
            if (err != null) return error(err);
            String diffSummary = pro.sketchware.ai.engine.diff.BlockDiffSupport.buildAfterDiff(
                    ctx, scId, "delete_block", beforeText);
            String msg = "Block " + blockId + " deleted from event '" + fullName + "'";
            return success(diffSummary != null ? msg + "\n" + diffSummary : msg);
        }
    }
