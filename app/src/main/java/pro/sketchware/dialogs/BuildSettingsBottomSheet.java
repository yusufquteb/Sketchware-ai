...
public class BuildSettingsBottomSheet extends BottomSheetDialogFragment {
    ...
    public static void handleJavaVersionChange(String choice) {
        if (choice.equals(SETTING_JAVA_VERSION_1_7)) {
            // Java 1.7 is not actually usable for compilation: Sketchware-generated code
            // always uses lambda expressions, which require Java 8+. The build backend
            // silently compiles 1.7 projects as 1.8 (see ProjectBuilder.buildEcjArguments()).
            // Warn the user here instead of letting that happen invisibly.
            SketchwareUtil.toast("Java 1.7 is not fully supported: the build system will compile using Java 8 compatibility mode.");
        } else {
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        // Default false: prevents the LogSender foreground service (Connected to Code On The Go)
        // from appearing automatically. User can still enable it manually.
        setCheckboxValue(binding.cbEnableLogcat, SETTING_ENABLE_LOGCAT, false);
