...
public class ResourcesEditorActivity extends BaseAppCompatActivity {
    ...
    private void initializeBackgroundTask(String variant) {
        ...
        setupViewPager();
        // Delay the background task until after the first layout pass so that
        // ViewPager2 has time to attach the fragments (onAttach/onCreate).
        // Without this, the background thread reads activity.variant while
        // the fragment's activity field is still null → NullPointerException.
        binding.viewPager.post(this::startBackgroundTask);
    }
    ...
    private void setupViewPager() {
        // Only create the adapter once; re-creating it would detach existing fragments
        // while a background task still holds a reference → NPE on activity.variant
    ...
    public void onDestroy() {
        ...
        if (pendingSearch != null) {
            searchDebounceHandler.removeCallbacks(pendingSearch);
        }
