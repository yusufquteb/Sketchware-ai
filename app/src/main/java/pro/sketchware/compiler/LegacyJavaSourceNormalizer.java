 ...
 *
 * The temp tree is always rebuilt from scratch (wipe, recopy, renormalize)
 * rather than incrementally synced. An earlier version tried to only copy
 * files that looked new/modified (comparing lastModified() timestamps, and
 * later file content) to save time, but that meant a stale copy could
 * silently survive here even when the real generated source had changed --
 * e.g. after toggling Material3 Manager off/on, which changes generated
 * imports without necessarily producing a timestamp/size difference the
 * sync would reliably notice. Since ProjectBuilder now performs a full Java
 * recompilation on every build anyway, there is no compilation-time benefit
 * to incremental syncing here, only correctness risk, so it has been
 * removed. Wiping and recopying a project's generated Java sources (a
 * handful of small files) takes a negligible amount of time.
 */
