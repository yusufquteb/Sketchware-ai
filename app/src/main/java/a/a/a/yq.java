...
public class yq {
    ...
    public void generateDebugFiles(Context context) {
        // SketchLogger / LogSender connects to "Code On The Go" IDE at runtime and shows a
        // persistent foreground notification ("LogSender Service – Connected to Code On The Go").
        // Default is now FALSE so new projects are quiet unless the user explicitly enables
        // "Enable Logcat" in Build Settings.
        boolean logcatEnabled = N.isDebugBuild && new BuildSettings(sc_id).getValue(
                        BuildSettings.SETTING_ENABLE_LOGCAT, BuildSettings.SETTING_GENERIC_VALUE_FALSE)
    ...
    public ArrayList<SrcCodeBean> a(hC projectFileManager, eC projectDataManager, BuiltInLibraryManager builtInLibraryManager) {
        ...
        //
        // SketchwareUtil.java is normally only (re)generated when missing, so
        // hand-edited copies survive rebuilds -- same policy as Activities and
        // layouts above. BUT its generated content is not toggle-independent:
        // Lx.i() conditionally emits "import com.google.android.material.color
        // .MaterialColors;" plus a call to MaterialColors.getColor(...) when
        // Material3 was enabled at generation time. If the user later disables
        // Material3 Manager, the file already exists so this block is skipped
        // entirely, leaving a stale import/call to a class that Material3 being
        // off removes from the classpath -> NoClassDefFoundError the instant
        // any generated code touches SketchwareUtil (which is essentially
        // immediately, since it backs toasts/utility calls used everywhere) ->
        // instant crash on launch. Re-enabling Material3 afterwards does not
        // self-heal this, because the file still exists and the check above
        // still skips it.
        //
        // Fix: only skip generation when the file exists AND its Material3-
        // dependent content already agrees with the current toggle state. This
        // still protects genuine hand-edits (skipped whenever the marker
        // already matches, which is the common case), while forcing a refresh
        // exactly when the toggle flips.
        File sketchwareUtilFile = new File(javaDir + "SketchwareUtil.java");
        boolean material3Enabled = material3LibraryManager.isMaterial3Enabled();
        if (!javaFiles.contains(sketchwareUtilFile)
                || isSketchwareUtilStaleForMaterial3State(sketchwareUtilFile, material3Enabled)) {
            srcCodeBeans.add(new SrcCodeBean("SketchwareUtil.java",
                    Lx.i(packageName, material3Enabled)));
        }
    ...
    /**
     * Returns true if an existing SketchwareUtil.java on disk was generated
     * for the opposite Material3 state from the one currently configured,
     * meaning it needs to be regenerated (see the call site for the crash
     * this prevents). Detected by checking for the "MaterialColors" marker
     * that {@link Lx#i(String, boolean)} only emits when Material3 is
     * enabled: its presence should always agree with the current toggle.
     *
     * Returns false (i.e. "leave the file alone") if the file can't be read,
     * since that's the pre-existing behavior and safer than forcing an
     * unrelated rewrite based on missing information.
     */
    private boolean isSketchwareUtilStaleForMaterial3State(File file, boolean material3Enabled) {
        String content = FileUtil.readFileIfExist(file.getAbsolutePath());
        if (content == null || content.isEmpty()) {
            return false;
        }
        boolean referencesMaterialColors = content.contains("com.google.android.material.color.MaterialColors");
        return referencesMaterialColors != material3Enabled;
    }
