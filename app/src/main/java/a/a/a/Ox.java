...
public class Ox {
    ...
    private void writeBackgroundResource(XmlBuilder nx, ViewBean viewBean) {
        ...
        if (backgroundResource == null || "NONE".equalsIgnoreCase(backgroundResource)) {
            ...
            boolean hasExplicitBackgroundColor = viewBean.layout.hasBackgroundColor;

            if (hasExplicitBackgroundColor || backgroundResColor != null || backgroundColor != 0xffffff) {
    ...
    private void writeWidget(XmlBuilder nx, ViewBean viewBean) {
        ...
        if (viewBean.getClassInfo().b("WaveSideBar")) {
            ...
            if (resTextColor != null && !toNotAdd.contains("app:sidebar_text_color")) {
            ...
            } else if (textColor != 0 && textColor != 0xffffff && !toNotAdd.contains("app:sidebar_text_color")) {
    ...
    private void writeImgSrcAttr(XmlBuilder nx, ViewBean viewBean) {
        ...
        if (!resName.isEmpty() && !"NONE".equals(resName)) {
            ...
            boolean useCompatSrc = viewBean.image.useCompatSrc || nx.c().equals("FloatingActionButton");
            if (useCompatSrc) {
                if (!toNotAdd.contains("app:srcCompat") && !injectHandler.contains("srcCompat")) {
                    nx.addAttribute("app", "srcCompat", value);
                }
            } else {
                if (!toNotAdd.contains("android:src") && !injectHandler.contains("src") && !injectHandler.contains("srcCompat")) {
    ...
    private void writeTextAttributes(XmlBuilder nx, ViewBean viewBean) {
        ...
        if (viewBean.text.resTextColor != null || viewBean.text.textColor != 0xffffff) {
        ...
        switch (viewBean.type) {
            ...
            case ViewBeans.VIEW_TYPE_WIDGET_MULTIAUTOCOMPLETETEXTVIEW:
                ...
                if (viewBean.text.resHintColor != null || viewBean.text.hintColor != 0xffffff) {
