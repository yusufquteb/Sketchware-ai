...
public class Jx {
    ...
    private boolean isJava7build(String scId) {
    ...
    public String generateCode(boolean isAndroidStudioExport, String sc_id) {
        ...
        // ── Extra Library imports (Library Manager settings) ───────────────
        if (buildConfig.g) {
            ...
            if (extraSettings.isEdgeToEdge()) {
                ...
                // WindowCompat.setDecorFitsSystemWindows() is the programmatic,
                // compileSdk-independent way to opt out of Android 15+ forced edge-to-edge.
                addImport("androidx.core.view.WindowCompat");
            }
        ...
        if (isFragment) {
            ...
            if (buildConfig.g) {
                LibraryExtrasSettings extraSettings = new LibraryExtrasSettings(
                        new ProjectSettingsStore(SketchApplication.getContext(), sc_id));
                if (extraSettings.isEdgeToEdge()) {
                    // User opted IN: must be called BEFORE setContentView per AndroidX docs.
                    // Wrapped in try-catch to prevent NoClassDefFoundError on first
                    // build after update before the dex cache is refreshed.
                    sb.append("try { EdgeToEdge.enable(this); } catch (Throwable ignored) {}").append(EOL);
                }
                if (isViewBindingEnabled) {
                ...
                if (!extraSettings.isEdgeToEdge()) {
                    // User opted OUT: call AFTER setContentView.
                    // WindowCompat.setDecorFitsSystemWindows(window, true) is the programmatic,
                    // compileSdk-independent way to resist Android 15+ system-forced edge-to-edge.
                    // androidx.core is always present with AppCompat — no extra library needed.
                    sb.append("WindowCompat.setDecorFitsSystemWindows(getWindow(), true);").append(EOL);
                }
                if (extraSettings.isWindowInsetsHandling() && !isJava7build(sc_id)) {
        ...
        if (buildConfig.isFirebaseEnabled) {
            ...
            // ── Firebase-dependent extras init (MainActivity only) ────────
            if (!isFragment && buildConfig.g
                    && projectFileBean.getActivityName().equals("MainActivity")) {
                ProjectSettingsStore store = new ProjectSettingsStore(
                        SketchApplication.getContext(), sc_id);
                LibraryExtrasSettings extraInit = new LibraryExtrasSettings(store);
                if (extraInit.isUseGoogleAnalytics()) {
                    sb.append("FirebaseAnalytics.getInstance(getApplicationContext());").append(EOL);
                }
                if (extraInit.isUseOneSignal()) {
                    String oneSignalAppId = new OneSignalSettingsManager(store).getAppId();
                    if (oneSignalAppId == null || oneSignalAppId.trim().isEmpty()) {
                        oneSignalAppId = "YOUR_ONESIGNAL_APP_ID";
                    }
                    sb.append("OneSignal.initWithContext(getApplicationContext(), \"")
                      .append(oneSignalAppId.trim())
                      .append("\");").append(EOL);
