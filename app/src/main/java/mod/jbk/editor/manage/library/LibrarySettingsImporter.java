...
public class LibrarySettingsImporter {
    ...
    public void showDialog(Activity activity) {
        ...
        // app:lottie_colorFilter cannot take a "?attr/..." theme-attribute reference directly:
        // Lottie reads it via TypedArray.getResourceId(), which only returns a real resource id
        // for "@..." references. For "?attr/..." references it returns -1, and passing -1 to
        // AppCompatResources.getColorStateList() throws Resources$NotFoundException (0xffffffff).
        // Resolving the attribute manually here and applying it as a color filter avoids that.
        TypedValue colorPrimaryValue = new TypedValue();
        if (activity.getTheme().resolveAttribute(R.attr.colorPrimary, colorPrimaryValue, true)) {
            animationView.addValueCallback(
                    new com.airbnb.lottie.model.KeyPath("**"),
                    com.airbnb.lottie.LottieProperty.COLOR_FILTER,
                    frameInfo -> new android.graphics.PorterDuffColorFilter(
                            colorPrimaryValue.data, android.graphics.PorterDuff.Mode.SRC_ATOP));
        }
