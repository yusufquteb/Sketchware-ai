...
public class BuiltInLibraries {
    ...
    // Newer Kotlin-based SavedState API (SavedStateReader/Writer, serialization) required
    // by lifecycle-viewmodel-savedstate-android-2.10.0. Kept separate from
    // ANDROIDX_SAVEDSTATE (1.2.1) since AppCompat/Fragment/etc. only need the classes
    // already present in 1.2.1.
    ...
    // Upgraded from material-1.13.0 to material-1.14.0-beta01: this version ships
    // the full Widget/Theme/ThemeOverlay.Material3Expressive.* style family
    // natively (133 styles), so no separate compat/stub library is needed
    // anymore. This mirrors the known-working Sketchware Bro v7.1.0 build,
    // whose bundled libs.zip/dexs.zip contain material-1.14.0-beta01 and no
    // material3-expressive-compat artifact at all.
    // material-1.14.0: ships the full Widget/Theme/ThemeOverlay.Material3Expressive.*
    // style family natively (374 references confirmed in its compiled resources),
    // matching exactly what upstream Sketchware-Pro/Sketchware-Next ships in
    // assets/libs/{libs,dexs}.zip. No separate compat/stub library is needed.
    ...
    // ── Extra Libraries (from Library Manager extras) ─────────────────────
    // Media3 / ExoPlayer
    public static String ANDROIDX_MEDIA3_COMMON        = "media3-common-1.10.1";
    public static String ANDROIDX_MEDIA3_CONTAINER     = "media3-container-1.10.1";
    public static String ANDROIDX_MEDIA3_DATABASE      = "media3-database-1.10.1";
    public static String ANDROIDX_MEDIA3_DATASOURCE    = "media3-datasource-1.10.1";
    public static String ANDROIDX_MEDIA3_DECODER       = "media3-decoder-1.10.1";
    public static String ANDROIDX_MEDIA3_EXOPLAYER     = "media3-exoplayer-1.10.1";
    public static String ANDROIDX_MEDIA3_EXOPLAYER_HLS = "media3-exoplayer-hls-1.10.1";
    public static String ANDROIDX_MEDIA3_EXTRACTOR     = "media3-extractor-1.10.1";
    public static String ANDROIDX_MEDIA3_UI            = "media3-ui-1.10.1";
    ...
    // Credential Manager
    public static String ANDROIDX_BIOMETRIC           = "biometric-1.1.0";
    public static String ANDROIDX_CREDENTIALS         = "credentials-1.5.0";
    public static String ANDROIDX_CREDENTIALS_PLAY_AUTH = "credentials-play-services-auth-1.5.0";
    public static String GOOGLEID                     = "googleid-1.1.1";

    // Glide Transformations
    public static String GLIDE_TRANSFORMATIONS        = "glide-transformations-4.3.0";

    // Google Analytics (Firebase Analytics)
    public static String PLAY_SERVICES_MEASUREMENT_API = "play-services-measurement-api-23.0.0";
    ...
    public static final BuiltInLibrary[] KNOWN_BUILT_IN_LIBRARIES = {
            ...
            new BuiltInLibrary(ANDROIDX_LIFECYCLE_VIEWMODEL_SAVEDSTATE, List.of(ANDROIDX_ANNOTATION_JVM, ANDROIDX_CORE_KTX, ANDROIDX_LIFECYCLE_COMMON,
                    ...
                    ANDROIDX_SAVEDSTATE, ANDROIDX_SAVEDSTATE_ANDROID, JETBRAINS_KOTLIN_STDLIB,
            ...
            new BuiltInLibrary(ANDROIDX_SAVEDSTATE_ANDROID, List.of(ANDROIDX_SAVEDSTATE, ANDROIDX_CORE,
                    JETBRAINS_KOTLIN_STDLIB), "androidx.savedstate"),
            ...
            // ── Media3 / ExoPlayer ────────────────────────────────────────
            new BuiltInLibrary(ANDROIDX_MEDIA3_COMMON, List.of(ANDROIDX_ANNOTATION_EXPERIMENTAL,
                    JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_MEDIA3_CONTAINER, List.of(ANDROIDX_MEDIA3_COMMON,
                    JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_MEDIA3_DATABASE, List.of(ANDROIDX_MEDIA3_COMMON,
                    JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_MEDIA3_DATASOURCE, List.of(ANDROIDX_MEDIA3_COMMON,
                    ANDROIDX_MEDIA3_DATABASE, ANDROIDX_EXIFINTERFACE, JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_MEDIA3_DECODER, List.of(ANDROIDX_MEDIA3_COMMON,
                    JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_MEDIA3_EXTRACTOR, List.of(ANDROIDX_MEDIA3_COMMON,
                    ANDROIDX_MEDIA3_CONTAINER, ANDROIDX_MEDIA3_DECODER, JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_MEDIA3_EXOPLAYER, List.of(ANDROIDX_MEDIA3_COMMON,
                    ANDROIDX_MEDIA3_DATASOURCE, ANDROIDX_MEDIA3_EXTRACTOR,
                    JETBRAINS_KOTLIN_STDLIB), "androidx.media3.exoplayer"),
            new BuiltInLibrary(ANDROIDX_MEDIA3_UI, List.of(ANDROIDX_MEDIA3_COMMON,
                    ANDROIDX_RECYCLERVIEW, JETBRAINS_KOTLIN_STDLIB), "androidx.media3.ui"),
            new BuiltInLibrary(ANDROIDX_MEDIA3_EXOPLAYER_HLS, List.of(ANDROIDX_MEDIA3_EXOPLAYER,
                    JETBRAINS_KOTLIN_STDLIB)),

            // ── Shizuku ───────────────────────────────────────────────────
            new BuiltInLibrary(DEV_RIKKA_SHIZUKU_AIDL),
            new BuiltInLibrary(DEV_RIKKA_SHIZUKU_SHARED, List.of(DEV_RIKKA_SHIZUKU_AIDL,
                    JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(DEV_RIKKA_SHIZUKU_API, List.of(DEV_RIKKA_SHIZUKU_AIDL,
                    DEV_RIKKA_SHIZUKU_SHARED, JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(DEV_RIKKA_SHIZUKU_PROVIDER, List.of(ANDROIDX_ANNOTATION_JVM,
                    DEV_RIKKA_SHIZUKU_API)),

            // ── Android Billing ───────────────────────────────────────────
            new BuiltInLibrary(ANDROID_BILLING, List.of(ANDROIDX_ACTIVITY,
                    PLAY_SERVICES_BASE, PLAY_SERVICES_BASEMENT, PLAY_SERVICES_TASKS)),

            // ── Credential Manager ────────────────────────────────────────
            new BuiltInLibrary(GOOGLEID, List.of(JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_BIOMETRIC, List.of(JETBRAINS_KOTLIN_STDLIB)),
            new BuiltInLibrary(ANDROIDX_CREDENTIALS_PLAY_AUTH),
            new BuiltInLibrary(ANDROIDX_CREDENTIALS, List.of(ANDROIDX_BIOMETRIC,
                    ANDROIDX_CREDENTIALS_PLAY_AUTH, GOOGLEID,
                    JETBRAINS_KOTLIN_STDLIB), "androidx.credentials"),

            // ── Glide Transformations ─────────────────────────────────────
            new BuiltInLibrary(GLIDE_TRANSFORMATIONS, List.of(GLIDE)),

            // ── Google Analytics (Firebase Analytics) ─────────────────────
            new BuiltInLibrary(PLAY_SERVICES_MEASUREMENT_API, List.of(
                    PLAY_SERVICES_ADS_IDENTIFIER, PLAY_SERVICES_BASEMENT,
                    PLAY_SERVICES_MEASUREMENT_BASE, PLAY_SERVICES_MEASUREMENT_SDK_API,
                    PLAY_SERVICES_TASKS, FIREBASE_COMMON, FIREBASE_COMPONENTS,
                    FIREBASE_IID, FIREBASE_MEASUREMENT_CONNECTOR,
                    JETBRAINS_KOTLIN_STDLIB)),
    ...
    /**
     * Forces a full re-extraction of the built-in libraries' resources/assets
     * (libs.zip), regardless of what hasFileChanged() would otherwise report.
     * <p>
     * Intended for UI-driven recovery, e.g. the "Create" action in DesignActivity's
     * "Missing directory detected" dialog. Previously that action only called
     * mkdirs() on the single missing directory (e.g. a library's res/ folder),
     * which created an empty directory with no actual resource content — aapt2
     * would then fail differently, or silently compile with no resources, rather
     * than actually fixing the problem. Deleting the on-disk copy of libs.zip here
     * makes hasFileChanged() detect a mismatch on the next check and re-extract the
     * full archive, restoring every built-in library's files, not just one folder.
     *
     * @return true if the marker was cleared (or didn't need to be) and re-extraction
     *         can proceed on the next build; false if the archive itself was missing
     *         or couldn't be cleared, which would indicate a more serious app-data
     *         problem outside the scope of this recovery action.
     */
    public static boolean forceReextractLibraries() {
        File libsArchiveOnDisk = new File(EXTRACTED_COMPILE_ASSETS_PATH, "libs.zip");
        if (libsArchiveOnDisk.exists() && !libsArchiveOnDisk.delete()) {
            return false;
        }
        try {
            extractCompileAssets();
            return true;
        } catch (Exception e) {
            android.util.Log.e("BuiltInLibraries", "forceReextractLibraries: extraction failed: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void extractCompileAssets(@NonNull BuildProgressReceiver... progressReceivers) {
        ...
        // Defensive check: hasFileChanged() only compares the libs.zip archive itself.
        // If extraction was ever interrupted (app killed mid-build, storage full, etc.)
        // while the archive on disk still matched, the marker would report "unchanged"
        // on the next run and skip re-extraction entirely, leaving individual library
        // folders (e.g. shizuku-shared-13.1.5) missing their classes.jar and causing D8
        // to fail with NoSuchFileException at compile time instead of at this earlier,
        // more diagnosable point. If the libs directory is missing or empty, force one
        // re-extraction attempt regardless of what the archive comparison said.
        File libsDir = new File(libsDirectoryPath);
        File[] libsDirContents = libsDir.exists() ? libsDir.listFiles() : null;
        if (libsDirContents == null || libsDirContents.length == 0) {
            android.util.Log.w("BuiltInLibraries", "Built-in libraries directory is missing or empty "
                    + "after extraction check; forcing re-extraction of " + libsArchiveName);
            fileUtil.b(libsDirectoryPath);
            fileUtil.f(libsDirectoryPath);
            new KB().a(libsArchivePath, libsDirectoryPath);
        }
