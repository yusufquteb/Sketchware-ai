...
## Phase 6 (تابع 3) — إصلاح المساعد الأوفلاين المعطّل فعليًا: كتالوج الموديلات كان لسه litertlm

بعد نجاح بناء CI ودمج migration الـ engine (llama.cpp) و16KB alignment fix، اختبار المستخدم
الفعلي على شاشة AI Settings كشف إن المساعد الأوفلاين **معطّل بالكامل رغم إن كل الأجزاء
الظاهرة (البناء، الشاشة، قائمة الموديلات) شكلها طبيعي**. السبب الجذري: `LocalModelCatalog.java`
فضل بيأشّر على ملفات `.litertlm` من `litert-community` — رغم إن `LlamaCppEngineBridge` و
`LocalModelProvider` كانوا اتحولوا بالكامل لـ llama.cpp من جلسة سابقة، والمحرك ده بيحمّل
`.gguf` فقط. النتيجة: أي تنزيل موديل كان بينجح (الروابط القديمة لسه شغالة) لكن التحميل في
المحرك كان هيفشل بالكامل بصمت — خلل غير مكتشف كان هيوصل لكل مستخدم يجرب المساعد الأوفلاين.

**السبب في توقف الإصلاح من قبل**: huggingface.co محجوب على مستوى الشبكة في هذه البيئة (403
policy denial)، وده يمنع التحقق المباشر من أسماء ملفات GGUF ومقاساتها — والكتالوج نفسه موثّق
فيه تاريخ حقيقي من روابط متخيَّلة أدّت لأخطاء 404 (راجع تاريخ الملف قبل هذا التعديل)، فمينفعش
نكرر نفس الغلطة على مستوى الكتالوج كله.

**الحل**: المستخدم فتح وأكّد 6 روابط ريبو GGUF حقيقية من جهازه مباشرة، واستُخدم `WebSearch`
للتحقق التقاطعي من أسماء الملفات/الأحجام الدقيقة مقابل كل ريبو مؤكَّد (تفاصيل مستوى الثقة لكل
إدخال موثّقة في javadoc الملف نفسه). الكتالوج اتكتب من جديد بالكامل: 6 موديلات (نفس العائلات
السابقة) بروابط GGUF حقيقية (بمعظمها Q8_0)، وحذف حقل الـ context-size لكل موديل لأنه بقى قيمة
تحميل واحدة ثابتة (8192) في `LlamaCppEngineBridge` بدل ما يكون مخبوز في اسم كل ملف.

**نتيجة الاختبار الفعلي بعد الدمج**: موديل واحد (Qwen2.5 1.5B، المؤكَّد بأعلى ثقة) نزل بنجاح.
اتنين من الموديلات اللي أسماء ملفاتهم كانت "مُستنتَجة من نمط تسمية" لا "مؤكَّدة بلقطة مباشرة"
(Qwen3 0.6B و Qwen2 0.5B Instruct) رجّعوا HTTP 404 فعلاً — تأكيد حي إن التمييز بين درجات
الثقة في javadoc الكتالوج كان له أساس واقعي، وليس مجرد حيطة زيادة. إصلاح الاسمين ينتظر رابط
الملف الدقيق من المستخدم (نفس انضباط "لا تخمين" اللي الملف موثّق بيه من قبل).

## Phase 6 (تابع 4) — حذف تلقائي لمحادثات المساعد العائم عند الخروج من التطبيق

طلب المستخدم إن محادثات `AiAssistantBottomSheet` (المساعد العائم اللي بيتفتح من شاشات زي
Manage Sound/Image وCompile Log) متتراكمش تلقائياً — كانت مصمَّمة قبل كده تتحفظ للأبد وتترجع
عند إعادة الفتح ("dismiss→reopen preserves history")، بعكس تبويب Agent المستقل اللي المفروض
يفضل يحتفظ بتاريخه.

**التنفيذ**: بدون أي اعتمادية Gradle جديدة (`androidx.lifecycle:lifecycle-process` مش موجودة
في المشروع أصلاً) — نمط `Application.ActivityLifecycleCallbacks` كلاسيكي: عداد Activities
شغّالة، وأي مرة يوصل الصفر بيتحدد فحص مؤجَّل (700ms)؛ لو لسه صفر بعد المهلة (يعني مفيش Activity
جديدة بدأت — الانتقال العادي بين الشاشات بيتداخل زمنياً، الجديدة بتبدأ قبل ما القديمة توقف)
فده معناه التطبيق خرج فعلاً للخلفية. عند التأكد، `ConversationManager.
deleteAllAssistantSheetConversations()` (دالة جديدة) بتمسح بس المجلدات اللي بادئة
بـ`"assistant_"` — نفس الـ namespace اللي `AiAssistantBottomSheet` بيستخدمه — من غير ما تلمس
محادثات تبويب Agent المستقل خالص (بيستخدم namespace مختلف تماماً).

**ملاحظة جانبية من نفس النقاش**: تأكّد إن مخزن المعرفة المشترك (`KnowledgeStore`/
`KnowledgeRetriever`) **بالفعل مربوط** بالمساعدين الأونلاين والأوفلاين معاً (`AgentExecutor`
و`LocalModelProvider`)، عكس تعليق قديم في `KnowledgeRetriever.java` بيقول "لسه مش مربوط" —
تعليق متجاوَز من جلسة سابقة. أما معلومات `Workspace` (تجميع المشاريع) فمش بتتغذّى في المخزن
ده خالص حالياً — اتوضّح للمستخدم كفجوة حقيقية لكن مؤجَّلة لحد ما يقرر لو مفيدة له.

## Phase 6 (تابع 5) — تقليص حجم مكتبات llama.cpp الأصلية بدل التراجع عن الترحيل بالكامل

المستخدم لاحظ إن نسخ الـ APK اللي فيها llama.cpp (arm64-v8a/x86_64) ضخمة جداً (330-345MB)
مقابل النسخ اللي من غيرها (150MB)، واقترح التراجع الكامل عن llama.cpp والرجوع لـ LiteRT-LM
لتقليل الحجم. **الرد**: التراجع الكامل هيرجّع المشكلة الأصلية اللي بدأت الجلسة كلها (سقف
4096 توكن ثابت مخبوز في كل ملف `.litertlm`، سبب "الأوفلاين ما بيستجيبش للأدوات"). بدل كده،
تتبّعنا السبب الحقيقي للانتفاخ لغاية كود ggml نفسه (`third_party/llama.cpp/ggml/src/
CMakeLists.txt`): علم `GGML_CPU_ALL_VARIANTS=ON` بيخلّي ggml يبني نسخة منفصلة من مكتبة الـ
CPU backend لكل نوع معالج — **~7 نسخ على arm64-v8a** (android_armv8.0_1 لغاية
android_armv9.2_2) **و~14 نسخة على x86_64** (x64 لغاية sapphirerapids) — كلها add_library()
منفصلة، وده هو سبب الحجم مش llama.cpp نفسه.

**الحل المطبَّق** (`llama/build.gradle`): `GGML_CPU_ALL_VARIANTS=OFF` + بناء نسخة واحدة فقط
لكل معمارية بمعايير حديثة معقولة:
- **arm64-v8a**: `GGML_CPU_ARM_ARCH=armv8.2-a+dotprod+fp16` — نفس الخط الأساسي اللي منطق
  ggml الداخلي بيختاره أصلاً لما DOTPROD وFP16_VECTOR_ARITHMETIC مفعّلين، بيغطي الغالبية
  العظمى من أجهزة أندرويد arm64 من 2018+.
- **x86_64**: `GGML_SSE42/AVX/AVX2/F16C/FMA/BMI2=ON` — معيار مكافئ لـ "haswell" (معالجات
  2013+)، شائع جداً على المحاكيات والأجهزة الحديثة.
- `GGML_BACKEND_DL=ON` فضل زي ما هو — آلية `ggml_backend_load_all_from_path` في
  `ai_chat.cpp` بتشتغل بنفس الطريقة سواء لقت ملف `.so` واحد في المجلد أو سبعة.

النتيجة المتوقَّعة: انخفاض كبير في حجم نسخ arm64-v8a/x86_64 مع الحفاظ الكامل على سقف 8192
توكن وإصلاح مشكلة الأدوات الأصلية — بدون التراجع عن أي شغل من الترحيل. **لم يُختبر فعلياً
بعد** — البناء الحقيقي على CI هو التحقق الأول.

**النتيجة الفعلية (تأكيد المستخدم على جهازه)**: arm64-v8a من 338MB إلى 289MB،
x86_64 من 345MB إلى 281MB — تحسّن حقيقي (~50MB) لكن أقل من المتوقَّع، لأن حجم مكتبات
llama.cpp نفسها (حتى بنسخة CPU واحدة) لسه أكبر من "الأساسي" (~150MB) بفارق كبير.

## Phase 6 (تابع 6) — تقليص إضافي: نسختين 64-bit بس + حذف رموز تصحيح المكتبات الأصلية

المستخدم طلب: (أ) إصدار نسختين فقط (arm64-v8a وx86_64) بدل الخمسة الحالية، و(ب) تقريب
الحجم أكتر من 150MB.

**(أ) تقليل مخرجات الـ APK** (`app/build.gradle` splits.abi):
- `include "arm64-v8a", "x86_64"` بدل الأربعة (armeabi-v7a/arm64-v8a/x86/x86_64).
- `universalApk false` بدل `true` — النسخة الشاملة (كانت 418-531MB) كانت بتجمع مكتبات
  llama.cpp لكل المعماريات الأربعة في ملف واحد رغم إن وحدة `:llama` أصلاً بتُبنى لـ
  arm64-v8a/x86_64 بس (`llama/build.gradle`'s `ndk.abiFilters`) — نسخ armeabi-v7a/x86 ما
  كانتش فيها المساعد الأوفلاين من الأساس، وأجهزة 32-bit أصبحت نادرة جداً.

**(ب) حذف رموز التصحيح (debug symbols) من المكتبات الأصلية** (`app/build.gradle`
buildTypes.debug/release):
```
ndk { debugSymbolLevel = "NONE" }
```
AGP بيحتفظ افتراضياً بجداول رموز DWARF كاملة داخل كل ملف `.so` (حتى مع
`CMAKE_BUILD_TYPE=Release` اللي بيتحكم في تحسين الكومبايلر بس مش في حذف الرموز) — وده حصة
كبيرة من الحجم في كود C++ مبني بكثافة templates/inlining زي ggml/llama.cpp. المشروع مالوش
